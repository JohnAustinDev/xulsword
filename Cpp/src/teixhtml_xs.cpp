/******************************************************************************
 *
 *  teixhtml_xs.cpp - Derived from osisxhtml_xs.cpp. Render filter for classed XHTML of a TEI module.
 *
...
BasicFilterUserData *TEIXHTMLXS::createUserData(const SWModule *module, const SWKey *key) {
  return new MyUserDataXS(module, key);
}


TEIXHTMLXS::TEIXHTMLXS() {
  setTokenStart("<");
  setTokenEnd(">");
  ...
  // unique to TEIXHTMLXS
  addAllowedEscapeString("nbsp");
}

class TEIXHTMLXS::TagStack : public std::stack<SWBuf> {
};

TEIXHTMLXS::MyUserDataXS::MyUserDataXS(const SWModule *module, const SWKey *key) : BasicFilterUserData(module, key), quoteStack(new TagStack()), hiStack(new TagStack()), titleStack(new TagStack()), lineStack(new TagStack()), htmlTagStack(new TagStack()), pStack(new TagStack()) {
  inXRefNote    = false;
  suspendLevel = 0;
...
TEIXHTMLXS::MyUserDataXS::~MyUserDataXS() {
  delete quoteStack;
  delete hiStack;
  delete titleStack;
  delete lineStack;
...
char TEIXHTMLXS::processText(SWBuf &text, const SWKey *key, const SWModule *module) {
  char *from;
  char token[4096];
  int tokpos = 0;
  bool intoken = false;
...
bool TEIXHTMLXS::handleToken(SWBuf &buf, const char *token, BasicFilterUserData *userData) {
  ...
  // manually process if it wasn't a simple substitution
    ...
    // BEGIN TEI SPECIFIC TAGS
    // <sense>
    if (!strcmp(tag.getName(), "sense")) {
    ...
    // END TEI SPECIFIC TAGS
    
    // BEGIN TAGS FROM OSIS FILTER
    // <w> tag
