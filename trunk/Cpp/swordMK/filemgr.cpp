		...
		if (*loop == file) {
			...
			if ((!access(file->path, 04)) || ((file->mode & O_CREAT) == O_CREAT)) {	// check for at least file exists / read access before we try to open
				...
				for (int i = 0; i < tries; i++) {
					...
					file->fd = wn_open(file->path, file->mode|O_BINARY, file->perms);
...
signed char FileMgr::trunc(FileDesc *file) {
	...
	if (writable) {
		...
		int fd = wn_open(buf, O_CREAT|O_RDWR, S_IREAD|S_IWRITE|S_IRGRP|S_IROTH);
		if (fd < 0)
			return -3;
		...
		if (size < 1) {
			// zero out the file
			::close(file->fd);
			file->fd = wn_open(file->path, O_TRUNC, S_IREAD|S_IWRITE|S_IRGRP|S_IROTH);
			::close(file->fd);
...
int FileMgr::openFileReadOnly(const char *fName) {
	int fd = wn_open(fName, O_RDONLY|O_BINARY, S_IREAD|S_IWRITE|S_IRGRP|S_IROTH);
	return fd;
}
...
int FileMgr::createPathAndFile(const char *fName) {
	...
	fd = wn_open(fName, O_CREAT|O_WRONLY|O_BINARY, S_IREAD|S_IWRITE|S_IRGRP|S_IROTH);
	if (fd < 1) {
		createParent(fName);
		fd = wn_open(fName, O_CREAT|O_WRONLY|O_BINARY, S_IREAD|S_IWRITE|S_IRGRP|S_IROTH);
	}
...
int FileMgr::copyFile(const char *sourceFile, const char *targetFile) {
	...
	if ((sfd = wn_open(sourceFile, O_RDONLY|O_BINARY, S_IREAD|S_IWRITE|S_IRGRP|S_IROTH)) < 1)
		return -1;
