...
int FileMgr::sysOpen(FileDesc *file) {
	...
	for (loop = &files; *loop; loop = &((*loop)->next)) {
		...
		if (*loop == file) {
			...
			if ((!sw_access(file->path, 04)) || ((file->mode & O_CREAT) == O_CREAT)) {	// check for at least file exists / read access before we try to open
...
signed char FileMgr::existsFile(const char *ipath, const char *ifileName)
{
	...
	signed char retVal = !sw_access(path, 04);
	delete [] path;
	return retVal;
}
