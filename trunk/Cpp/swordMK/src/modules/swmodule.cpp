...
// Begin mod...
	...
	*this = TOP;
	indexerKey->setText(this->getKeyText());
	setKey(*saveKey);
// End mod...
...
signed char SWModule::createSearchFramework(void (*percent)(char, void *), void *percentUserData, double maxTime, int *percentComplete) {
#ifdef USELUCENE
	SWKey *saveKey = 0;
	SWKey *searchKey = 0;
	SWKey textkey;
	...
	for (OptionFilterList::iterator filter = optionFilters->begin(); filter != optionFilters->end(); filter++) {
		...
		if (!strcmp("Hebrew Vowel Points", (*filter)->getOptionName())) {
			(*filter)->setOptionValue("Off");
		}
		if (!strcmp("Hebrew Cantillation", (*filter)->getOptionName())) {
			(*filter)->setOptionValue("Off");
		}
	}
	...
	SWKey *vsystemk = CreateKey();
	VerseKey *vsystem = SWDYNAMIC_CAST(VerseKey, vsystemk);
	if (VerseKey * tmp = SWDYNAMIC_CAST(VerseKey, searchKey)) {tmp->setVersificationSystem(vsystem?vsystem->getVersificationSystem():WESTERN);}
  ...
  target.Assign(getConfigEntry("AbsoluteDataPath"));
	...
	if (vkcheck) {vkcheck->setVersificationSystem(vsystem?vsystem->getVersificationSystem():WESTERN);}
	...
	*this = BOTTOM;
	...
	//*this = TOP;
	key->setText(indexerKey->getText());
	
	VerseKey chapMax;
	chapMax.setVersificationSystem(vsystem?vsystem->getVersificationSystem():WESTERN);
	if (vsystem) {delete(vsystem);}
	SWBuf proxBuf;
	SWBuf proxLem;
	SWBuf strong;
  ...
  bool checkTime = (maxTime > 0) ? true:false;
  time_t startTime;
  time_t lastTime;
  time(&startTime);
	time(&lastTime);
	
	while (!err && (!checkTime || difftime(lastTime, startTime)<maxTime)) {
...
//fflush(stdout);
  ...
  indexerKey->setText(key->getText());
	...
	for (OptionFilterList::iterator filter1 = optionFilters->begin(); filter1 != optionFilters->end(); filter1++) {
		(*filter1)->setOptionValue(*origVal++);
	}
