#!/usr/bin/perl
# Usage: localeCodes.pl MK MKS MKO
...
while (<NFLE>) {
  if ($_ =~ /^(\S+).*\s+(.*)\.locale\.manifest/) {$localeManifestCRC{$2} = $1;}
  if ($_ =~ /^(\S+).*\s+(.*)\.jar/) {$jarManifest{$2} = $1;}
}
close (NFLE);
...
foreach $locale (sort keys %localeManifestCRC) {
  ...
  open (OUTF1, ">$MKS\\localeDev\\locales\\$locale.txt");
  open (OUTF2, ">$MK\\xulrunner\\chrome\\$locale.txt");
