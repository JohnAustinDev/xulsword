...
// TextClasses is an object used to parse information from element titles. Many of these are
// generated by libxulsword. NOTE: the sr, dt, and dtl class may have multiple ";" or " " 
// separated references in its title, but only the first is matched. The TextClasses object 
// is mainly used by the getElementInfo function below to retrieve any relevant info
// from an arbitrary DOM element. DOM elements must have their class and title pattern(s)
// included in TextClasses for its info to be retreivable. The TextClasses class must always 
// be the first class in the elements class list, and only the part before "-" defines its
// TextClasses class.
const TextClasses = {
  vs:     [ { re:new RegExp(/^(([^\.]+)\.(\d+)\.(\d+))\.([^\.]+)$/),                              bk:2,    ch:3,     vs:4,    lv:4,     mod:5, osisref:1 } ],
  fn:     [ { re:new RegExp(/^(\d+)\.(([^\.]+)\.(\d+)\.(\d+))\.([^\.]+)$/),                 nn:1, bk:3,    ch:4,     vs:5,    lv:5,     mod:6, osisref:2 } ],
  cr:     [ { re:new RegExp(/^(\d+)\.(([^\.]+)\.(\d+)\.(\d+))\.([^\.]+)$/),                 nn:1, bk:3,    ch:4,     vs:5,    lv:5,     mod:6, osisref:2 } ],
  un:     [ { re:new RegExp(/^([^\.]+)\.(([^\.]+)\.(\d+)\.(\d+))\.([^\.]+)$/),             res:1, bk:3,    ch:4,     vs:5,    lv:5,     mod:6, osisref:2 } ],
  sr:     [ { re:new RegExp(/^(([^\.]+)\.(\d+)\.(\d+))\.([^\.]+)(;|$)/),                          bk:2,    ch:3,     vs:4,    lv:4,     mod:5, osisref:1 },
            { re:new RegExp(/^(([^\.]+)\.(\d+)\.(\d+)\s*-\s*[^\.]+\.\d+\.(\d+))\.([^\.]+)(;|$)/), bk:2,    ch:3,     vs:4,    lv:5,     mod:6, osisref:1 } ],
  dt:     [ { re:new RegExp(/^(\S+)\:(\S+)[^\.]*\.([^\.]+)$/),                        refentry:2, bk:null, ch:2,    vs:null, lv:null, mod:1 } ],
  dtl:    [ { re:new RegExp(/^(\S+)\:(\S+)[^\.]*\.([^\.]+)$/),                        refentry:2, bk:null, ch:2,    vs:null, lv:null, mod:1 } ],
  par:    [ { re:new RegExp(/^(\d+)\.([^\.]+)$/),                                          par:1, bk:null, ch:null, vs:1,    lv:1,     mod:2 } ],
  fnlink: [ { re:new RegExp(/^(\d+)\.(([^\.]+)\.(\d+)\.(\d+))\.([^\.]+)$/),                 nn:1, bk:3,    ch:4,     vs:5,    lv:5,     mod:6, osisref:2 } ],
...
// Lists of footnotes, usernotes, and cross-references (like those returned by libxulsword) have titles matching this:
const XSNOTE = new RegExp(/src\.(fn|cr|un)\.(([^\.]+)\.(\w+)\.(\d+)\.(\d+)\.(\w+))/);
