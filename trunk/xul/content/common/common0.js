...
// function which uses LibSword. Otherwise unhandled exceptions may 
...
// functions in this file cannot rely on the program wide globals which 
// are defined in MainWindow.
...
function getLocaleBundle(locale, file) {
  ...
  var bundle = BUNDLESVC.createBundle("chrome://xulsword/locale/" + file, LOCALESVC.getLocaleFromAcceptLanguage(locale));
...
function dString(x, locale) {
  var localnumbers = null;
  if (typeof(MainWindow) == "undefined") localnumbers = DisplayNumeral;
  else localnumbers = MainWindow.DisplayNumeral;
  
  if (!locale) locale = getLocale();
  
  var s = String(x);
  if (!localnumbers[locale]) getDisplayNumerals(locale, localnumbers);
  s = s.replace("0", localnumbers[locale][0],"g");
  s = s.replace("1", localnumbers[locale][1],"g");
  s = s.replace("2", localnumbers[locale][2],"g");
  s = s.replace("3", localnumbers[locale][3],"g");
  s = s.replace("4", localnumbers[locale][4],"g");
  s = s.replace("5", localnumbers[locale][5],"g");
  s = s.replace("6", localnumbers[locale][6],"g");
  s = s.replace("7", localnumbers[locale][7],"g");
  s = s.replace("8", localnumbers[locale][8],"g");
  s = s.replace("9", localnumbers[locale][9],"g");
  return s;
}
...
function iString(x, locale) {
  ...
  var s = String(x);
  if (!localnumbers[locale]) getDisplayNumerals(locale, localnumbers);
  s = s.replace(localnumbers[locale][0], "0", "g");
  s = s.replace(localnumbers[locale][1], "1", "g");
  s = s.replace(localnumbers[locale][2], "2", "g");
  s = s.replace(localnumbers[locale][3], "3", "g");
  s = s.replace(localnumbers[locale][4], "4", "g");
  s = s.replace(localnumbers[locale][5], "5", "g");
  s = s.replace(localnumbers[locale][6], "6", "g");
  s = s.replace(localnumbers[locale][7], "7", "g");
  s = s.replace(localnumbers[locale][8], "8", "g");
  s = s.replace(localnumbers[locale][9], "9", "g");
  return s;
}
...
// This should be called during onload of every .xul or .html window. It
// creates the CSS classes for locales and modules and adds them to the
// current stylsheet. It also sets the attributes used by CSS to control
