...
test(
  16,
  LibSword.getModuleInformation('KJV', 'GlobalOptionFilter'),
  'OSISLemma<nx>OSISStrongs<nx>OSISMorph<nx>OSISFootnotes<nx>OSISHeadings<nx>OSISRedLetterWords'
);
...
test(
  48,
  Location.convertLocation('KJV', 'Matt.3.5.8', 'Synodal'),
  'Matt.3.5.8'
);
jsdump('\n');
...
test(
  51,
  Location.convertLocation('KJV', 'Ps.22.9.12', 'Synodal'),
  'Ps.21.10.13'
);
jsdump('\n');
...
test(
  54,
  Location.convertLocation('Synodal', 'Ps.21.10.13', 'KJV'),
  'Ps.22.9.12'
);
jsdump('\n');
...
test(
  55,
  Location.convertLocation('Synodal', '1Kgs.6.1-1Kgs.6.18', 'KJV'),
  '1Kgs.6.1.18'
);
jsdump('\n');
...
test(
  56,
  Location.convertLocation('Synodal', 'Ps.114.1 - Ps.114.2', 'KJV'),
  'Ps.116.1.2'
);
jsdump('\n');
...
test(
  59,
  Location.convertLocation('Synodal', 'Ps 113:2-7', 'KJV'),
  'Ps.114.2.7'
);
jsdump('\n');
...
test(
  60,
  Location.convertLocation('Synodal', 'Ps 113:10-Ps 113:20', 'KJV'),
  'Ps.115.2.12'
);
jsdump('\n');
...
test(
  98,
  LibSword.search('KJV', 'be.*t.*', 'John', 0 /* regex */, 0, true),
  296
);
test(
  99,
  LibSword.getSearchResults('KJV', 2, 1, false).match(/title=\"[^\"]+\"/g)[0],
  'title="John.1.7.KJV"'
);
...
for (
  let index = 0;
  index < LibSword.search('KJV', 'word', 'John', 1, 0, true);
  index++
) {
  let searchResult = LibSword.getSearchResults(
    'KJV',
    index,
    1,
    false,
...
test(
  101,
  LibSword.search('KJV', 'shineth in darkness', 'John', 2 /* multi */, 0, true),
  1
);
...
test(
  103,
  LibSword.getSearchResults('KJV', 0, 1, false).match(/title=\"[^\"]+\"/g)[0],
  'title="John.1.5.KJV"'
);
...
function test(testNum, result, expectedResult) {
  if (result == expectedResult)
    jsdump(`Test #:${  testNum  } Passed (${  result  })\n`);
  else {
    jsdump(
      `Test #:${
        testNum
...
jsdump(
  `getDictionaryEntry БАШАН=${  LibSword.getDictionaryEntry('UZDOT', 'БАШАН')}`
);
jsdump(`getAllDictionaryKeys=${  LibSword.getAllDictionaryKeys('UZDOT')}`);
jsdump(
  `getGenBookTableOfContents=${  LibSword.getGenBookTableOfContents('Pilgrim')}`
);
jsdump(
  `getGenBookChapterText=${
    LibSword.getGenBookChapterText('Pilgrim', '/PART II/PREFACE')}`
);
