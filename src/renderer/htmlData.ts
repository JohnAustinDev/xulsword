...
export function libSwordData2XulswordData(dataIn: LibSwordHTMLData): HTMLData {
  ...
  if (type && osisref) {
    ...
    if (m) {
      locationGB = {
        module: m[0],
        key: decodeOSISRef(m[1]), // required for dt, dtl. Others??
      };
    }
  } else if (mod && key) {
  ...
  if (!bk && osisref) {
    const x = osisref.split('.');
    if (Object.values(C.SupportedBooks).some((bg: any) => bg.includes(x[0]))) {
      bk = x[0] as OSISBookType;
      if (x[1] && !Number.isNaN(Number(x[1]))) {
        ch = Number(x[1]);
      }
      if (x[2] && !Number.isNaN(Number(x[2]))) {
        vs = Number(x[2]);
      }
      if (x[3] && !Number.isNaN(Number(x[3]))) {
        lv = Number(x[3]);
      }
    }
  }
