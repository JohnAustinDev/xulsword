...
//jsdump("Activating Popup in " + window.name + ":" + elem.className + ", " + e.type);    
    ...
    if (p && p.mod && this.selectRef.hasOwnProperty(p.mod)) {
      ...
      if (referenceBible && G.Tab.hasOwnProperty(referenceBible) && G.Tab[referenceBible].modType == DICTIONARY) {
        var aref = getCompanionModules(referenceBible);
        if (aref.length && G.Tab.hasOwnProperty(aref[0])) referenceBible = aref[0];
      }
    }
    ...
    case "un":
      ...
      if (w == 0) {
        ...
        G.LibSword.getChapterText(p.mod, p.bk + " " + p.ch);
        Texts.footnotes[w] = G.LibSword.getNotes();
      }
    ...
    if (p && p.mod && (/^(cr|sr)$/).test(type)) {
      ...
      for (var t=0; t<G.Tabs.length; t++) {
        if (G.Tabs[t].modType == BIBLE) bmods.push(G.Tabs[t].modName);
      }
    ...
    if ((/^(sn)$/).test(type)) {
      for (var sls in G.SpecialModules.LanguageStudy) {
        if (G.SpecialModules.LanguageStudy[sls].length < 2) continue; // need no button if only one choice
        ...
        this.appendModSelect(
          popupheader,
          G.SpecialModules.LanguageStudy[sls], 
  ...
  this.appendModSelect = function(parent, mods, selectMod, isFeature, arg2) {
    ...
    for (var m=0; m<mods.length; m++) {
      if (!G.Tab[mods[m]]) continue;
      var option = select.appendChild(document.createElement("option"));
      option.className = "cs-" + G.Tab[mods[m]].locName;
      option.setAttribute("value", mods[m]);
      if (mods[m] == selectMod) option.setAttribute("selected", "selected");
      option.textContent = G.Tab[mods[m]].label;
    }
