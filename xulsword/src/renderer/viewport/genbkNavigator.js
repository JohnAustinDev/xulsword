...
G.GenBookNavigator = {
  ...
  ChapterResource: G.BM.RDF.GetResource("http://www.xulsword.com/tableofcontents/rdf#Chapter"),
  ...
  getDatasource: function(mod) {
    if (!this.Datasources.hasOwnProperty(mod)) {
      ...
      writeSafeFile(rdfFile, G.LibSword.getGenBookTableOfContents(mod), true);
      
      var myURI = encodeURI("File://" + rdfFile.path.replace(/\\/g, "/"));
      //jsdump("Adding: " + myURI.match(/\/([^\/]+\.rdf)/)[1] + "\n");
      this.Datasources[mod] = G.BM.RDF.GetDataSourceBlocking(myURI);
    }
...
//jsdump("getGenBookInfo.firstUnpinnedKey=" + uneval(ret.firstUnpinnedKey));
  ...
  update: function(info) {
	...
	catch(er) {jsdump("G.GenBookNavigator.update failed!: " + er); return 0;}
    var removeDS = [];
  ...
  select: function(rdfChapter) {
    ...
    var i = this.TreeBuilder().getIndexOfResource(G.BM.RDF.GetResource(rdfChapter));
    if (i == -1) return false;
    
    this.Tree().view.selection.select(i);  
    
    window.setTimeout(function () {G.GenBookNavigator.Tree().boxObject.ensureRowIsVisible(i);}, 0);
  ...
  showRdfChapter: function(rdfChapter) {
    ...
    do {
      ...
      var index = this.TreeBuilder().getIndexOfResource(G.BM.RDF.GetResource(sub));
    ...
    try {
      var res = G.BM.RDF.GetResource(rdfChapter);
