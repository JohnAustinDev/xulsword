  ...
  // Otherwise it's a windowed viewport
    ...
    for (var w=1; w<=NW; w++) {
      if (w == towindow) {
        ...
        if (G.Tab[ViewPort.Module[w]].modType != DICTIONARY) {
...
function ViewPortObj(viewPortObj) {
  ...
  this.drawTabs = function(w) {
    ...
    orig.setAttribute("title", (!G.Tab.ORIG_NT && !G.Tab.ORIG_OT ? " style=\"display:none;\"":""));

    var tabs = document.getElementById("tabs" + w);
    for (var t=0; t<G.Tabs.length; t++) {

      // insert ORIG tab after BIBLEs
      if (G.Tabs[t].modType != BIBLE && orig) {
        tabs.appendChild(orig);
        orig = null;
      }
      
      var tab = tabs.appendChild(document.createElement("input"));
      tab.setAttribute("class", "tab tab" + G.Tabs[t].tabType);
      tab.setAttribute("id", "w" + w + ".tab.norm." + t);
      tab.setAttribute("type", "button");
      tab.setAttribute("value", G.Tabs[t].label);
      tab.setAttribute("title", G.Tabs[t].description);
    }
    ...
    for (t=0; t<G.Tabs.length; t++) {
      var option = select.appendChild(document.createElement("option"));
      option.setAttribute("class", "tab tab" + G.Tabs[t].tabType);
      option.setAttribute("id", "w" + w + ".tab.mult." + t);
      option.textContent = G.Tabs[t].label;
    }
  ...
  document.getElementById("viewportbody").setAttribute("hasOriginalLanguage", (G.Tab.ORIG_OT || G.Tab.ORIG_NT ? "true":"false"));
  ...
  if (viewPortObj) {
    ...
    for (var w=1; w<=NW; w++) {
      ...
      if (!G.Tab.hasOwnProperty(this.Module[w])) this.Module[w] = prefs.getCharPref("DefaultVersion");
      
      if (G.SpecialModules.DailyDevotion.hasOwnProperty(this.Module[w])) this.Key[w] = "DailyDevotionToday";
      
      if (!G.Tab.ORIG_NT && !G.Tab.ORIG_OT) this.ShowOriginal[w] = false;
    ...
    for (w=1; w<=NW; w++) {
      for (var t=0; t<G.Tabs.length; t++) {
        var inhide = new RegExp("(^|;)" + escapeRE(G.Tabs[t].modName) + ";");
        if (inhide.test(getPrefOrCreate("w" + w + ".hidden", "Char", ""))) G.Tabs[t]["w" + w + ".hidden"] = true;
        else G.Tabs[t]["w" + w + ".hidden"] = false;
      }
    }
    ...
    if (this != XS_window.ViewPort) {
      for (var w=1; w<=NW; w++) {
        if (document.getElementById("tabs" + w).getAttribute("moduleType") != "none" && window.frameElement) {
          window.frameElement.ownerDocument.title = fixWindowTitle(G.Tab[this.Module[w]].label) + 
              (G.Tab[this.Module[w]].description ? ": " + G.Tab[this.Module[w]].description:"");
          break;
        }
      }
    }
    ...
    for (var w=1; w<=NW; w++) {
      ...
      if (w > this.NumDisplayedWindows || 
        ...
        if ((w+1) <= this.NumDisplayedWindows && 
            ...
            G.Tab[this.Module[w]].modType == BIBLE &&
            !this.ShowOriginal[w] && 
            ...
            document.getElementById("text" + (w+1)).getAttribute("moduleType") != "none" &&
            (G.Tab[this.Module[w]].modType == COMMENTARY || G.Tab[this.Module[w]].modType == GENBOOK) &&
            this.IsPinned[w] == this.IsPinned[(w+1)] && 
            this.Module[w] == this.Module[Number(w+1)])
            value = "show2";
      ...
      //if ((G.ModuleConfigs[this.Module[w]].direction == "rtl" || G.ProgramConfig.direction == "rtl" )&& (/^show(2|3)$/).test(value)) value = "show1";
      ...
      var mtype = getShortTypeFromLong(G.Tab[this.Module[wThis]].modType);
      ...
      var prev = true;
      var next = true;
      switch(G.Tab[this.Module[wThis]].modType) {
        case BIBLE:
        case COMMENTARY:
      ...
      t.setAttribute("textdir", G.ModuleConfigs[this.Module[wThis]].direction);
...
//for (w=1; w<=NW; w++) {jsdump("w=" + w + ", value=" + document.getElementById("text" + w).getAttribute("columns"));}
    ...
    for (w=1; w<=NW; w++) {
      ...
      for (var t=0; t<G.Tabs.length; t++) {
        ...
        if (G.Tabs[t].modName == this.Module[w]) {
        ...
        if (G.Tabs[t]["w" + w + ".hidden"]) {
          ...
          if (G.Tabs[t].modType == BIBLE) noBibleTabs = false;
        }
    ...
    for (w=1; w<=NW; w++) {
      ...
      for (var t=0; t<G.Tabs.length; t++) {
        if (G.Tabs[t]["w" + w + ".hidden"]) continue;
      ...
      if (t >= G.Tabs.length-1) document.getElementById("w" + w + ".multitab").style.display = "none";
      else {
        ...
        if (t == G.Tabs.length) document.getElementById("w" + w + ".tabselect").setAttribute("active", "false");
...
//jsdump("UPDATING VIEW PORT h=" + winh);
    ...
    // General-G.Book Chooser (part of xulsword.xul but treated as part of XS_window.ViewPort)
    var genbkinfo = G.GenBookNavigator.getGenBookInfo();
  ...
  this.disableMissingBooks = function(hide) {
    ...
    for (var b=0; b<G.Book.length; b++) {
      ...
      for (var a=0; books && a<books.length; a++) {
        if (books[a] == G.Book[b].sName) {have=true; break;}
      }
  ...
  this.resize = function() {
    ...
    self.resizeInterval = window.setInterval(function() {
      if (typeof(G.LibSword) != "object" || !G.LibSword.libSwordReady("viewport.resizeFunc")) return;
  ...
  this.unload = function() {
    ...
    for (var w=1; w<=NW; w++) {
      var hide = "";
      for (var t=0; t<G.Tabs.length; t++) {
        if (G.Tabs[t]["w" + w + ".hidden"]) hide += G.Tabs[t].modName + ";";
      }
