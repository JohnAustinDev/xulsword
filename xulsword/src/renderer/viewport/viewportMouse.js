...
function tabMouse(e) {
  ...
  case "click":
    ...
    if (p[2] == "orig" && (G.Tab[ViewPort.Module[w]].modType != BIBLE || document.getElementById("viewportbody").getAttribute("hasOriginalLanguage") == "false")) return;
    if (p[2] == "orig") ViewPort.ShowOriginal[w] = !ViewPort.ShowOriginal[w];
    else ViewPort.selectTab(w, G.Tabs[t].modName);
...
//G.Tabs are html and not XUL, but we want XUL tool tips to appear above them (for
...
function openTabToolTip(t, w, cX, cY) {
  ...
  if (t !== null) {
    var modName = G.Tabs[t].modName;
    if (!modName) return;
    
    var desc = G.Tabs[t].description;
    if (!desc) return;
    
    desc = desc.substr(0, TOOLTIP_LEN);
    if (desc.length==TOOLTIP_LEN) desc += "...";
    
    tt.firstChild.setAttribute("class", "cs-" + G.Tab[modName].locName);
  }
  else {
...
//jsdump("type:" + type + " title:" + elem.title + " class:" + elem.className + "\n");
  ...
  case "fn":
    if (w && G.Tab[ViewPort.Module[w]].modType == GENBOOK) okay = null; // genbk fn are embedded in text
...
function scriptClick(e) {
  ...
  if (w && !ViewPort.IsPinned[w] && G.Tab[ViewPort.Module[w]].modType == GENBOOK) {
    var key = ViewPort.Key[w];
    if (G.GenBookNavigator.selectedRdfChapter() != "rdf:#/" + ViewPort.Module[w] + key) {
      G.GenBookNavigator.select("rdf:#/" + ViewPort.Module[w] + key);
    }
  }
...
//jsdump("w:" + w + " type:" + type + " p:" + uneval(p) + ", elem_cn=" + elem.className + ", elem_t=" + elem.title);
  ...
  case "dtl":
  case "popupBackLink":
    if (w && elem.className && elem.className.indexOf('x-target_self') !== -1 && G.Tab[ViewPort.Module[w]].modType == DICTIONARY) {
  ...
  case "sbwin":
    ...
    openWindowXS("chrome://xulsword/content/viewport/viewport.xul", "viewport", p, null, wintop);
    break;

  case "prevchaplink":
    if (!w) break;
    var mod = ViewPort.Module[w];
    switch (G.Tab[mod].modType) {
    case BIBLE:
    ...
    case GENBOOK:
      ...
      // if not, then load previous chapter
        ...
        if (ViewPort.IsPinned[w]) {
          ...
          Texts.pinnedDisplay[w].Key = prevchap.match(G.GenBookNavigator.RDFCHAPTER)[2];
          Texts.update(SCROLLTYPEPREVIOUS, HILIGHTSKIP);
        }
        else G.GenBookNavigator.select(prevchap);
  ...
  case "nextchaplink":
    ...
    case GENBOOK:
      ...
      if (sb.scrollLeft == prev) {
        ...
        if (ViewPort.IsPinned[w]) {
          Texts.pinnedDisplay[w].scrollTypeFlag = SCROLLTYPETOP;
          Texts.pinnedDisplay[w].Key = nextchap.match(G.GenBookNavigator.RDFCHAPTER)[2];
          Texts.update(SCROLLTYPEPREVIOUS, HILIGHTSKIP);
        }
        else G.GenBookNavigator.select(nextchap);
      }
      break;
    }
    break;
  ...
  case "crref":
    G.Location.setLocation(p.mod, p.bk + "." + p.ch + "." + p.vs + "." + p.lv);
    XS_window.Texts.update(SCROLLTYPECENTER, HILIGHT_IFNOTV1);
    break;
    
  case "origoption":
    var orig = elem.getAttribute("value").split(".");
    if (findBookNum(orig[0]) < NumOT) G.Tab.ORIG_OT = G.Tab[orig[3]];
    else G.Tab.ORIG_NT = G.Tab[orig[3]];
...
var MouseWheel = {
  ...
  scroll: function(event) {
    ...
    if (G.Tab[ViewPort.Module[w]].modType == DICTIONARY) return;
    ...
    MouseWheel.SWTO = window.setTimeout(function () {MouseWheel.scroll2();}, 250);
  },
  ...
  scroll2: function() {
    ...
    if (G.Tab[ViewPort.Module[MouseWheel.SWwin]].modType == GENBOOK) {
    ...
    // else scrolling versekey modules
      ...
      else G.Location.setLocation(p.mod, p.bk + "." + p.ch + "." + p.vs);
    }
...
function previousPage(w) {
  ...
  if (ViewPort.IsPinned[w]) {
    ...
    G.Location.setLocation(v.mod, v.bk + "." + v.ch + "." + v.vs);
...
function nextChapterPinned(w) {
  ...
  if (chn < G.LibSword.getMaxChapter(Texts.pinnedDisplay[w].mod, Texts.pinnedDisplay[w].bk)) {
    chn++;
  }
  else return;
