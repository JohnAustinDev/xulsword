...
GenBookTexts = {
  ...
  read: function(w, d) {
    ...
    ret.htmlText = G.LibSword.getGenBookChapterText(d.mod, d.Key);
  ...
  updateAudioLinks: function(w) {
    ...
    for (var i = 0; i < icons.length; ++i) {
      ...
      if (G.AudioDirs.length && false) icons[i].className += " hasAudio"; //XS_window.getAudioForChapter(p.mod, p.bk, p.ch)
    }
  },
  ...
  firstRdfChapter: function(mod) {
    ...
    var root = G.BM.RDF.GetResource("rdf:#" + "/" + mod);
    
    var db = G.GenBookNavigator.getDatasource(mod);
    if (!db) {
      jsdump("ERROR: firstRdfChapter- No database for \"" + mod + "\"");
      return null;			
    }
    
    var chapter1 = db.GetTarget(root, G.BM.RDFCU.IndexToOrdinalResource(1), true);
    if (!chapter1) {
  ...
  previousRdfChapter: function(rdfChapter) {
    ...
    var test = parent.match(G.GenBookNavigator.RDFCHAPTER);
    if (!test) parent = null;

    // try previous node
    if (parent) {
      G.BM.RDFC.Init(ch.ds, G.BM.RDF.GetResource(parent));
      var siblings = G.BM.RDFC.GetElements();
    ...
    if (previous && G.BM.RDFCU.IsContainer(ch.ds, G.BM.RDF.GetResource(previous))) {
      G.BM.RDFC.Init(ch.ds, G.BM.RDF.GetResource(previous));
      var chldrn = G.BM.RDFC.GetElements();
      var last = null;
    ...
    if (previous && !previous.match(G.GenBookNavigator.RDFCHAPTER)[2]) return null;
  ...
  nextRdfChapter: function(rdfChapter, skipChildren) {
    ...
    // try first child...
    if (!skipChildren && G.BM.RDFCU.IsContainer(ch.ds, ch.node)) {
      G.BM.RDFC.Init(ch.ds, ch.node);
      var chldrn = G.BM.RDFC.GetElements();
    ...
    // return null if result is not an rdfChapter
    if (next && !next.match(G.GenBookNavigator.RDFCHAPTER)[2]) return null;
    return next;
  },
  ...
  validateKeys: function() {
    for (var w=1; w<=ViewPort.NumDisplayedWindows; w++) {
      ...
      if (G.Tab[aMod].modType != GENBOOK) continue;
      ...
      if (ViewPort.Key[w]) {
        ...
        var chNode = G.BM.RDF.GetResource(chValue);
        var ch = G.GenBookNavigator.getDatasource(aMod).GetTarget(chNode, G.GenBookNavigator.ChapterResource, true);
      ...
      if (!validKey) {
        ...
        ViewPort.Key[w] = (ch && G.GenBookNavigator.RDFCHAPTER.test(ch) ? ch.match(G.GenBookNavigator.RDFCHAPTER)[2]:"");
//jsdump("validateKeys setting mod:" + aMod + " to key:" + ViewPort.Key[w]);
  ...
  getRdfChapterResource: function(rdfChapter) {
    ...
    var mod = rdfChapter.match(G.GenBookNavigator.RDFCHAPTER)[1];
    if (!mod) return r;
    
    r.ds = G.GenBookNavigator.getDatasource(mod);
    if (!r.ds) return r;
    ...
    while (es.hasMoreElements()) {
      ...
      if (e.ValueUTF8 == rdfChapter) {
        ...
        if (G.BM.RDFCU.IsContainer(r.ds, r.node)) break;
      }
    }
    
    // return null if this is not an rdfChapter
    if (r.node && !r.node.ValueUTF8.match(G.GenBookNavigator.RDFCHAPTER)[2]) r.node = null;
