...
//jsdump("scrollTypeFlag=" + scrollTypeFlag + ", hilightFlag=" + hilightFlag + ", force=" + force);
    ...
    for (var w=1; w<=NW; w++) {
      ...
      var modType = G.Tab[ViewPort.Module[w]].modType;
      ...
      if (this.scrollTypeFlag == SCROLLTYPETOP && 
        ...
        G.Location.setVerse(prefs.getCharPref("DefaultVersion"), 1, 1);
      }
...
/*    
    ...
    for (var w=1; w<=NW; w++) {
      //if (G.Tab[ViewPort.Module[w]].modType != BIBLE) continue;
      if (document.getElementById("text" + w).getAttribute("columns") == "hide") continue;
...
//jsdump(XS_window.document.getElementById("main-viewport").contentDocument.getElementsByTagName("HTML")[0].outerHTML);
  ...
  updateBible: function(w, display, force) {
    ...
    var t = document.getElementById("text" + w);
    var ltr = (G.ModuleConfigs[display.mod].direction == "ltr");
    var sb = t.getElementsByClassName("sb")[0];
...
//jsdump("reading w" + w + ": " + c);
        ...
        var c = Number(display.ch) + 1;
        while (c <= G.LibSword.getMaxChapter(d2.mod, d2.bk + "." + d2.ch)) {
//jsdump("reading w" + w + ": " + c);
          d2.ch = c;
          var tip = BibleTexts.read(w, d2);
          if (tip.htmlText.length <= 32) break; // stop if chapter is missing
...
//jsdump("reading w" + w + ": " + display.ch);
  ...
  updateGenBook: function(w, display, force) {
    ...
    if (!this.display[w] || this.display[w].mod != display.mod) {
      ...
      display.Key = (ch && G.GenBookNavigator.RDFCHAPTER.test(ch) ? ch.match(G.GenBookNavigator.RDFCHAPTER)[2]:"");
    }
  ...
  updateDictionary: function(w, display, force) {
    ...
    // reset key if the module has changed since last read
    if (!this.display[w] || this.display[w].mod != display.mod) {
      display.Key = (G.SpecialModules.DailyDevotion.hasOwnProperty(display.mod) ? "DailyDevotionToday":"");
    }
...
//jsdump("B-" + w + ":" + fs + ", " + elem.offsetWidth + ", " + w);
  ...
  getPageLinks: function() {
    var config = G.LocaleConfigs[getLocale()];
  ...
  getUserNotes: function(bk, ch, mod, text) {
    ...
    var usesVerseKey = (G.Tab[mod].modType == BIBLE || G.Tab[mod].modType == COMMENTARY);
    
    // Search User Data for notes with this book, chapter, and version
    var recs = G.BMDS.GetAllResources();
    while (recs.hasMoreElements()) {
      var res = recs.getNext();
      if (!G.ResourceFuns.isItemChildOf(res, G.BM.AllBookmarksRes, G.BMDS)) continue;
      if (G.BM.RDFCU.IsContainer(G.BMDS, res)) {continue;}
      
      // only show bookmarks which are usernotes
      var note = G.BMDS.GetTarget(res, G.BM.gBmProperties[NOTE], true);
      if (!note) continue;
      ...
      try {var module = G.BMDS.GetTarget(res, G.BM.gBmProperties[MODULE], true).QueryInterface(Components.interfaces.nsIRDFLiteral).Value;} catch (er) {continue;}
      if (module != mod) continue;
      
      // is this note in the chapter?
      try {var chapter = G.BMDS.GetTarget(res, G.BM.gBmProperties[CHAPTER], true).QueryInterface(Components.interfaces.nsIRDFLiteral).Value;} catch (er) {continue;} 
      if (usesVerseKey) {
        if (chapter != String(ch)) continue;
        try {var book = G.BMDS.GetTarget(res, G.BM.gBmProperties[BOOK], true).QueryInterface(Components.interfaces.nsIRDFLiteral).Value;} catch (er) {continue;}
        if (book != bk) continue;
      }
      else {
      ...
      try {var verse = Number(G.BMDS.GetTarget(res, G.BM.gBmProperties[VERSE], true).QueryInterface(Components.interfaces.nsIRDFLiteral).Value);} catch (er) {continue;}
      try {var lastverse = Number(G.BMDS.GetTarget(res, G.BM.gBmProperties[LASTVERSE], true).QueryInterface(Components.interfaces.nsIRDFLiteral).Value);} catch (er) {lastverse = verse;}
  ...
  getWindowDisplay: function(w) {
    ...
    return this.getDisplay(ViewPort.Module[w], G.Location.getLocation(ViewPort.Module[w]), display);
  },
  ...
  getDisplay: function(mod, loc, display) {
    ...
    for (var cmd in GlobalToggleCommands) {
      ...
      else display.globalOptions[GlobalToggleCommands[cmd]] = G.LibSword.getGlobalOption(GlobalToggleCommands[cmd]);
    }
...
//jsdump("v=" + uneval(getElementInfo(v)));
    ...
    if (G.Tab[d.mod].modType == COMMENTARY) {
    ...
    if (t.getAttribute("columns") == "show1") {
      ...
      case SCROLLTYPEENDSELECT:    // put selected verse at the end of the window or link, then select first verse of link or verse 1
        ...
        if (d.scrollTypeFlag == SCROLLTYPEENDSELECT) {
          // then set G.Location to first visible verse- but not implemented because not yet used
        }
        break;
      }
    }
    ...
    // or scroll multi-column windows...
      ...
      case SCROLLTYPENONE:         // don't scroll (for links this becomes SCROLLTYPECENTER)
      case SCROLLTYPECENTER:       // put selected verse in the middle of the window or link, unless verse is already entirely visible or verse 1
        var ltr = (G.ModuleConfigs[d.mod].direction == "ltr");
        ...
        if (d.scrollTypeFlag == SCROLLTYPEENDSELECT) {
          ...
          while(vs) {
            ...
            if (p && p.type == "vs" && vs.style && vs.style.display != "none") {
              G.Location.setLocation(p.mod, p.bk + "." + p.ch + "." + p.vs);
              break;
            }
            vs = vs.nextSibling;
          }
  ...
  isVisibleVerse: function(velem, w) {
    ...
    if (G.ModuleConfigs[ViewPort.Module[w]].direction == "ltr") {
      // we are LTR
