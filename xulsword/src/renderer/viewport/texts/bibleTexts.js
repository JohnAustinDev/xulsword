...
BibleTexts = {
  ...
  read: function(w, d) {
    ...
    for (var cmd in GlobalToggleCommands) {
      ...
      G.LibSword.setGlobalOption(GlobalToggleCommands[cmd], d.globalOptions[GlobalToggleCommands[cmd]]);
    }
    
    // get Bible chapter's text
    var un;
    if (d["ShowOriginal"]) {
      G.LibSword.setGlobalOption("Strong's Numbers", "On");
      G.LibSword.setGlobalOption("Morphological Tags", "On");
      
      // Get the appropriate original language module
      var mod2 = prefs.getCharPref("DefaultVersion");
      if (findBookNum(d.bk) < NumOT && G.Tab.ORIG_OT) {
        mod2 = G.Tab.ORIG_OT.modName;
      }
      else if (findBookNum(d.bk) >= NumOT && G.Tab.ORIG_NT) {
        mod2 = G.Tab.ORIG_NT.modName;
      }
      
      ret.htmlText = G.LibSword.getChapterTextMulti(d.mod + "," + mod2, d.bk + "." + d.ch).replace(/interV2/gm, "cs-" + mod2);
      
      G.LibSword.setGlobalOption("Strong's Numbers", prefs.getCharPref("Strong's Numbers"));
      G.LibSword.setGlobalOption("Morphological Tags", prefs.getCharPref("Morphological Tags"));
    }
    else {
...
//jsdump(ret.htmlText);
  ...
  getChapterHeading: function(d) {
    var l = G.ModuleConfigs[d.mod].AssociatedLocale;
    ...
    // if any portion of HTML returned by G.LibSword is not well-formed, then the entire page is broken.
    ...
    var lt = G.LibSword.getModuleInformation(d.mod, "NoticeLink");
    if (lt == NOTFOUND) lt = "";
    ...
    var html = "";
    html  = "<div class=\"chapterhead" + (d.ch==1 ? " chapterfirst":"") + " cs-" + l + ((/rtl/i).test(G.LocaleConfigs[l].direction) ? " RTL":"") + "\">";
    
    html +=   "<div class=\"chapnotice cs-" + d.mod + (!lt ? " empty":"") + "\">";
    html +=     "<div class=\"noticelink-c\">" + (lt ? lt:"") + "</div>";
    html +=     "<div class=\"noticetext\">"; // contains a span with class cs-mod because G.LibSword.getModuleInformation doesn't supply the class
    html +=       "<div class=\"cs-" + d.mod + "\">" + (lt ? G.LibSword.getModuleInformation(d.mod, "NoticeText"):"") + "</div>";
    html +=     "</div>";
    ...
    if (d["ShowOriginal"]) {
      var origs = G.SpecialModules.OriginalLanguages.Greek.concat(G.SpecialModules.OriginalLanguages.Hebrew);
      if (origs.length) {
        html += "<div class=\"origselect\">";
        html +=   "<select>";
        for (var i=0; i<origs.length; i++) {
          try {var selected = origs[i] == (findBookNum(d.bk) < NumOT ? G.Tab.ORIG_OT.modName:G.Tab.ORIG_NT.modName);}
          catch (er) {selected = false;}
          html +=   "<option class=\"origoption cs-" + G.Tab[origs[i]].locName + "\" value=\"" + d.bk + ".1.1." + origs[i] + "\"" + (selected ? " selected=\"selected\"":"") + ">" + G.Tab[origs[i]].label + "</option>";
        } 
        html +=   "</select>";
        html += "</div>";
      }
  ...
  getNotesHTML: function(notes, mod, gfn, gcr, gun, openCRs, w, keepTextNotes) {
    ...
    if (note) {
      ...
      for (var n=0; n < note.length; n++) {
        ...
        var lov = G.ModuleConfigs[mod].AssociatedLocale;
        if (lov == NOTFOUND) lov = getLocale();
        var modDirectionEntity = (G.ModuleConfigs[mod] && G.ModuleConfigs[mod].direction == "rtl" ? "&rlm;":"&lrm;");
        t +=   "<div class=\"fncol4\">";
        if (p.ch && p.vs) {
        ...
        case "fn":
          ...
          t += "<span class=\"fntext cs-" + (isASCII(body) ? DEFAULTLOCALE:mod) + (G.ModuleConfigs[mod].direction != G.ProgramConfig.direction ? " opposing-program-direction":"") + "\">" + body + "</span>";
          break;
        ...
        case "un":
          ...
          var unmod = null;
          try {
            unmod = G.BMDS.GetTarget(G.BM.RDF.GetResource(decodeURIComponent(p.nid)), G.BM.gBmProperties[NOTELOCALE], true);
            unmod = unmod.QueryInterface(Components.interfaces.nsIRDFLiteral).Value;
          }
          catch (er) {}
          var de = (unmod && G.ModuleConfigs[unmod] && G.ModuleConfigs[unmod].direction == "rtl" ? "&rlm;":"&lrm;");
  ...
  getRefHTML: function(w, mod, body, keepTextNotes) {
    ...
    var bk = G.Location.getBookName();
    var ch = G.Location.getChapterNumber(mod);
    var vs = 1;
    ...
    for (var i=0; i<ref.length; i++) {
      ...
      if (ref[i].indexOf("!") != -1) {
        ...
        html += "<span class=\"fntext cs-" + (isASCII(footnote) ? DEFAULTLOCALE:mod) + (G.ModuleConfigs[mod].direction != G.ProgramConfig.direction ? " opposing-program-direction":"") + "\">" + footnote + "</span>"
      ...
      if (!aVerse) aVerse = { text:"(" + ref[i] + " ??)", location:r.ref, tabNum:G.Tab[mod].index };
      if ((/^\s*$/).test(aVerse.text)) aVerse.text = "-----";
      
      var rmod = G.Tabs[aVerse.tabNum].modName;
      html += sep;
      html += "<a class=\"crref\" title=\"" + aVerse.location + "." + rmod + "\">";
      html += ref2ProgramLocaleText(aVerse.location);
      html += "</a>";
      html += "<span class=\"crtext cs-" + rmod + (G.ModuleConfigs[rmod].direction != G.ProgramConfig.direction ? " opposing-program-direction":"") + "\">";
  ...
  //dump(ref + "\n");
    ...
    if (m) {
      ...
      if (!(/Bible/i).test(m[1])) {
        if (G.Tab.hasOwnProperty(ret.mod) && G.Tab.hasOwnProperty(m[1])) {
          ref = G.LibSword.convertLocation(G.LibSword.getVerseSystem(m[1]), ref, G.LibSword.getVerseSystem(ret.mod));
        }
        else if (G.Tab.hasOwnProperty(m[1])) ret.mod = m[1];
        else {
          ret.mod = null;
    ...
    if ((/^[^\.]+\.\d+$/).test(ref)) {                            // bk.c
      if (ret.mod) ret.ref =  ref + ".1-" + ref + "." + G.LibSword.getMaxVerse(ret.mod, ref);
      else ret.ref = ref;
    }
  ...
  getIntroductions: function(mod, vkeytext) {
    if (!G.Tab[mod] || (G.Tab[mod].modType != BIBLE && G.Tab[mod].modType != COMMENTARY)) return { text:'', notes:'' };
    
    G.LibSword.setGlobalOption("Headings", "On");
    
    var intro = G.LibSword.getIntroductions(mod, vkeytext);
    var notes = G.LibSword.getNotes();
  
    G.LibSword.setGlobalOption("Headings", prefs.getCharPref("Headings"));
  ...
  updateAudioLinks: function(w) {
    ...
    for (var i = 0; i < icons.length; ++i) {
      ...
      if (G.AudioDirs.length && XS_window.getAudioForChapter(p.mod, p.bk, p.ch)) icons[i].className += " hasAudio";
    }
  }
