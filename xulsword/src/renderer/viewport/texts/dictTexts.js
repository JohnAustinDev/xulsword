...
DictTexts = {
  ...
  read: function(w, d) {
    ...
    if (!this.keyList[d.mod]) {
      this.keyList[d.mod] = G.LibSword.getAllDictionaryKeys(d.mod).split("<nx>");
      this.keyList[d.mod].pop();
      this.sortOrder = G.LibSword.getModuleInformation(d.mod, "LangSortOrder");
      if (this.sortOrder != NOTFOUND) {
        this.sortOrder += "0123456789";
        this.langSortSkipChars = G.LibSword.getModuleInformation(d.mod, "LangSortSkipChars");
  ...
  getEntryHTML: function(key, mods) {
    ...
    var html = "";
    if (mods.length == 1) {
      try {html = G.LibSword.getDictionaryEntry(mods[0], key);}
      catch (er) {jsdump("e1: missing key, trying uppercase..."); html = "";}
      if (!html) {
        try {html = G.LibSword.getDictionaryEntry(mods[0], key.toUpperCase());}
...
//jsdump("Raw entry:" + (html ? html:''));
      ...
      for (var dw=0; dw<mods.length; dw++) {
        var dictEntry="";
        try {dictEntry = G.LibSword.getDictionaryEntry(mods[dw], key);}
        catch (er) {jsdump("e2: missing key, trying uppercase..."); dictEntry = "";}
        if (!dictEntry) {
          try {dictEntry = G.LibSword.getDictionaryEntry(mods[dw], key.toUpperCase());}
...
//jsdump("Raw entry:" + (dictEntry ? dictEntry:''));
        if (dictEntry) {
          ...
          var dictTitle = G.LibSword.getModuleInformation(mods[dw], "Description");
  ...
  getLemmaHTML: function(strongsClassArray, matchingPhrase, sourcemod) {
    ...
    for (var i=0; i<strongsClassArray.length; i++) {
      ...
      if (info.key && info.mod) {
        ...
        var entry = G.LibSword.getDictionaryEntry(info.mod, info.key);
        if (entry) {
  ...
  replaceLinks: function(entry, mod) {
    ...
    if (link) {
      for (var x=0; x<link.length; x++) {
        ...
        var r = G.LibSword.getDictionaryEntry(mod, l[1].toUpperCase());
...
//var m=entry.match(/<\w+/g); if (m) {for (var x=0; x<m.length; x++) {if (!(/^<(table|tbody|span|sup|div|img|br|tr|td|h\d+|ul|li|b|i)$/i).test(m[x])) jsdump("INFO: Found unhandled tag \"" + m[x] + "\" in\n" + entry);}}
  ...
  getStrongsModAndKey: function(snclass) {
    ...
    case "S":
      ...
      if (res.mod) {
        for (var k=0; k<keys.length; k++) {
          try {if (G.LibSword.getDictionaryEntry(res.mod, keys[k])) break;}
    ...
    case "RM":
      ...
      if (G.SpecialModules.LanguageStudy.GreekParse.indexOf("Robinson") != -1) res.mod = "Robinson";
      break;
