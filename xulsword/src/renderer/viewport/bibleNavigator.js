...
BibleNavigator = {
  ...
  init: function() {
    if (G.LibSword.loadFailed) return;
  ...
  drawBooks: function(which, elem) {
    ...
    var end = (which == "ot" ? NumOT:G.Book.length);
  ...
  drawBook: function(elem, b) {
    ...
    div2.textContent = G.Book[b].bName;
  ...
  writeChapterMenu: function(elem, bk) {
    ...
    var row=1; 
    var col=1;
    for (var ch=1; ch<=G.LibSword.getMaxChapter("KJV", G.Book[bk].sName); ch++) {
      if (col == 1) {
 ...
 ***********************************************************************/  
  ...
  mouseHandler: function(e) {
    ...
    case "testament":
      ...
      case "click":
        G.Location.setLocation(ViewPort.firstDisplayBible(), G.Book[(p[1]=="ot" ? 0:NumOT)].sName + ".1.1");
        XS_window.Texts.update(SCROLLTYPECENTER, HILIGHTNONE);
        break;
      }
      break;
      
    // G.Book selector of the Bible Navigator
    case "book":
      switch (e.type) {  
      case MSOVER:
        ...
        break;
      case "click":
        G.Location.setLocation(ViewPort.firstDisplayBible(), G.Book[p[1]].sName + ".1.1");
        XS_window.Texts.update(SCROLLTYPECENTER, HILIGHTNONE);
        break;
      }
      break;
    ...
    case "chmenucell":
      ...
      case "click":
        G.Location.setLocation(ViewPort.firstDisplayBible(), G.Book[p[1]].sName + "." + p[2] + ".1.1");
        XS_window.Texts.update(SCROLLTYPECENTER, HILIGHTNONE);
        break
      }
      break;
    ...
    case "headlink":
      if (e.type == "click") {
        G.Location.setLocation(p[4], p[1] + "." + p[2] + "." + p[3]);
        XS_window.Texts.update(SCROLLTYPECENTER, HILIGHTNONE);
      }
      break;
  ...
  showHeadings: function(myid, screenY) {
    ...
    var chtxt = G.LibSword.getChapterText(biblemod, G.Book[p[1]].sName + "." + p[2]);
    ...
    if (head != null) {
      for (var h=0; h < head.length; h++) {
        ...
        if (!(/^<div[^>]*>\s*<\/div>$/).test(heading)) {
          ...
          a.id = "headlink_" + G.Book[p[1]].sName + "_" + p[2] + "_" + verse + "_" + biblemod;
    ...
    G.LibSword.setGlobalOption("Verse Numbers", prefs.getCharPref("Verse Numbers"));
  },
